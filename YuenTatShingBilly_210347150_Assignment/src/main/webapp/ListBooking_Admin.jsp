...
//                    out.print("<p class=\"font-semibold\">" + venueBookings.size() + "</p>");
    ...
    for (int x = 0; x < venueBookings.size(); x++) {
        ...
        if (vb.getStatus() == 0) {
            out.print("<span class=\"px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100\">Approved With No Payment</span>");

        } else if (vb.getStatus() == 1) {
        ...
        if (vb.getStatus() == 3 && vb.getPriority() == 1) {
            ...
            out.print("<td class=\"px-4 py-3 text-xs\">");
            out.print("<button class='px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple'>Approve Here!</button>");
            out.print("</td>");
            out.print("</form>");
        } else {
            out.print("<td class=\"px-4 py-3 text-xs\">");
        ...
        if (vb.getStatus() == 1) {
            if (vb.getCheckStatus() < 4) {
                out.print("<form action=ChangeCheck.jsp method=GET>");
                out.print("<input type=hidden name=userid value=" + request.getParameter("userid") + " />");
